    ...
    <div className="max-w-4xl mx-auto">
      ...
      {showCreate && (
        <div className="yinliu-card mb-6">
          <h3 className="font-medium mb-3">新建歌单</h3>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {playlists.map((pl) => (
          <div
            key={pl.id}
            className="group relative rounded-xl bg-[var(--bg-secondary)] border border-[var(--border)] overflow-hidden hover:border-[var(--accent)]/30 transition-all"
          >
            <a href={`/playlists?id=${pl.id}`} className="block">
              <div className="aspect-square bg-gradient-to-br from-primary-400/20 to-primary-600/20 flex items-center justify-center">
            ...
            {!pl.id.startsWith('sys_') && editingId !== pl.id && (
              <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <button
                  ...
                  className="p-1.5 rounded-lg bg-black/50 text-white hover:bg-red-500/80"
                >
            ...
            {/* Play button */}
            <button className="absolute bottom-14 right-3 p-2.5 rounded-full bg-[var(--accent)] text-white opacity-0 group-hover:opacity-100 transition-opacity shadow-lg hover:bg-[var(--accent-hover)]">
